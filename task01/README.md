...
* src/main/java/com/example/task01/Task01.java - исходный файл, который нужно модифицировать для выполнения задачи.
* src/test/java/com/example/task01/Tests.java - набор автоматических тестов, которые проверяют корректность выполнения задачи. (!) Тесты проверяют только формальные признаки выполнения задачи и не заменяют проверку качества выполнения преподавателем.
...
Запустите класс Tests через IDE ("Run tests") или с помощью maven ("mvn -pl task01 test").
